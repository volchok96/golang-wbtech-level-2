...
/*
Паттерн "Фасад" (Facade) — это структурный шаблон проектирования, который предоставляет простой интерфейс к сложной системе, 
скрывая её внутреннюю сложность. Основная идея паттерна заключается в том, чтобы создать объект (фасад), 
через который можно обращаться к различным компонентам или подсистемам системы, не взаимодействуя с ними напрямую.
